...
| --- | --- |
| `user.create` | `sourceUserId` (пароль WDTT), VK-хеши, порты, метка, срок, включённость |
| `user.update` | `sourceUserId`, обязательная ревизия, метка, срок, включённость, VK-хеши или порты |
...
Снимок содержит ограниченный набор полей: локальный ID, имя, пользовательскую метку, флаг доступа, срок, счётчики входящего/исходящего трафика, online, устройства и ревизию. `online` вычисляет локальная WDTT-панель по свежему WireGuard handshake. Private keys, конфигурации, токены и raw-метрики хоста в снимок не входят.
...
В исходную WDTT-панель нужно добавить только три opt-in модуля:
